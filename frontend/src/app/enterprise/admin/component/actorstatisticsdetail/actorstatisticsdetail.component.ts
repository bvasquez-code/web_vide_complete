...
@Component({
  selector: 'app-actorstatisticsdetail',
  templateUrl: './actorstatisticsdetail.component.html'
})
export class ActorstatisticsdetailComponent implements OnChanges {
  @Input() actorCod = '';
  ...
  async load(): Promise<void> {
    ...
    this.dailyViews = [];
    this.errorMessage = '';
    if (!this.actorCod) return;
    this.loading = true;
    try {
      const rpt = await this.adminVideoService.findActorStatisticsDetail(this.actorCod);
      if (rpt.ErrorStatus) {
        this.errorMessage = rpt.Message;
        return;
      }
      this.summary = rpt.Data?.Summary || null;
      this.topVideos = rpt.Data?.TopVideos || [];
      this.dailyViews = rpt.Data?.DailyViews || [];
    } catch {
      this.errorMessage = 'No se pudieron cargar las estadisticas del actor.';
    } finally {
      this.loading = false;
    }
  }
